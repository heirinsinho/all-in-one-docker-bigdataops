...
cond1 = F.sqrt(
    F.pow(static_poi.longitude - kafka_traffic.longitude, 2) + F.pow(static_poi.latitude - kafka_traffic.latitude,
                                                                     2)) < 0.1
df1 = static_poi.join(kafka_traffic, on=cond1).drop(kafka_traffic.longitude, kafka_traffic.latitude, kafka_traffic.id)

cond2 = F.sqrt(
    F.pow(static_poi.longitude - kafka_parkings.longitude, 2) + F.pow(static_poi.latitude - kafka_parkings.latitude,
                                                                      2)) < 0.1
df2 = static_poi.join(kafka_parkings, on=cond2).drop(kafka_parkings.longitude, kafka_parkings.latitude,
                                                     kafka_parkings.id)

cond3 = F.sqrt(
    F.pow(static_poi.longitude - kafka_bicimad.longitude, 2) + F.pow(static_poi.latitude - kafka_bicimad.latitude,
                                                                     2)) < 0.1
...
df = df.filter(df.occupancy.isNotNull()).groupBy("id", "longitude", "latitude").agg(
